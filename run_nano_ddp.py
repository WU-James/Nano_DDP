...
from nano_ddp import NanoDDPV1, NanoDDPV2, NanoDDPV3
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="BERT MRPC demo with NanoDDP V1, V2, or V3"
    )
    p.add_argument(
        "--path",
        default="v1",
        choices=sorted(_PATH_ALIASES),
        help="NanoDDP strategy: v1 (flat post-backward), v2 (per-param hooks), v3 (bucketed)",
    )
    p.add_argument(
        "--global-batch-size",
        type=int,
...
def main() -> None:
    args = _parse_args()
    path_key = _PATH_ALIASES[args.path]
    nano_ddp_cls = _NANO_DDP_BY_PATH[path_key]
    ...
    raw.to(device)
    model = nano_ddp_cls(raw, device=device)
    if dist.get_rank() == 0:
        print(f"NanoDDP {path_key.upper()} ({nano_ddp_cls.__name__})")
        print(
    ...
    if local_rank == 0:
        ...
        print(f"demo finished OK path={path_key.upper()} (wall time {elapsed:.2f}s)")
